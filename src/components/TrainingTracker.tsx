...
import { WorkoutTab } from './tabs/WorkoutTab.js';
import PlanTab from './tabs/PlanTab.js';
import HistoryTab from './tabs/HistoryTab.js';
import StatsTab from './tabs/StatsTab.js';

// 🔹 Opcjonalnie możesz wydzielić nagłówek, footer i nawigację w osobne komponenty

const TrainingTracker: React.FC = () => {
  const [activeTab, setActiveTab] = useState<TabType>('dashboard');

  // Hooki do zarządzania danymi
  const { workouts, measurements, setMeasurements, saveData, isLoading: storageLoading } = useStorage();
  const { allTemplates: templates, addTemplate, updateTemplate, deleteTemplate, duplicateTemplate, defaultTemplates, customTemplates } = useTemplates();

  // Hook do zarządzania bieżącym treningiem
  const { 
    currentWorkout, 
    startWorkout, 
    addSet, 
        ...
        <div className="bg-slate-800 rounded-xl shadow-2xl p-6">
          ...
          {activeTab === 'history' && (
            <HistoryTab
              ...
              onDelete={(id) => console.log('Usuń trening o id:', id)}
            />
          )}
